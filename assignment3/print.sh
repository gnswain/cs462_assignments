...
#initialize the building of the latex file
...
if [ sortbyCategory ]; then
    ...
    sort -n -k5 tmp2.txt > tmp3.txt
fi

while IFS=, read -r categoryAndItem cost quantity total; do

    category=$(echo $categoryAndItem | cut -d: f1 | tr -d)
    item=$(echo $categoryAndItem | cut -d: f2 | tr -d)
